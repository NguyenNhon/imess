...
class ChatFriendController : UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate{
    ...
    func getMessages() {
        let ref = FIRDatabase.database().reference().child("test").childByAutoId().child("messages")
        ref.observeSingleEvent(of: FIRDataEventType.value
            , with: { snapshot in
                if ( snapshot.value is NSNull ) {
                    //ref.childByAutoId().setValue(["name" : user?.displayName!, "email" : user?.email!, "photoUrl" : user?.photoURL?.path, "id" : uid!])
                    ref.childByAutoId().setValue(["id": "2", "content": "2", "members": "1"])
                }
        })
        //ref.setValue(["id": "1", "content": "1", "": "2"])
        //ref.childByAutoId().setValue(["id": "2", "content": "2", "members": "1"])
    }
